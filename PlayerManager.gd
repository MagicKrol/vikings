...
# ============================================================================
# 
# Purpose: Multi-player system coordination and resource management
# 
...
func _init(num_players: int = 4, region_mgr: RegionManager = null, map_gen: MapGenerator = null):
...
func _initialize_players() -> void:
	...
	for i in range(1, total_players + 1):
		...
		players[i] = player
		print("[PlayerManager] Initialized ", player.get_player_name(), " with starting resources")
...
func set_current_player(player_id: int) -> void:
	...
	if players.has(player_id):
		...
		print("[PlayerManager] Current player changed to ", get_current_player().get_player_name())
...
func next_player() -> Player:
	...
	print("[PlayerManager] Advanced to ", next_player.get_player_name())
	return next_player
...
func process_resource_income() -> void:
	"""Process resource income for human player only (Player 1)"""
	print("[PlayerManager] === Processing resource income for turn ===")
	
	# Only process income for Player 1 (human player) for now
	var player = players.get(1, null)
	if player != null:
		print("[PlayerManager] Processing income for ", player.get_player_name())
		_calculate_player_income(player)
	else:
		push_error("[PlayerManager] Player 1 not found!")
...
func _calculate_player_income(player: Player) -> void:
	...
	if region_manager == null:
		push_error("[PlayerManager] CRITICAL: RegionManager is null - cannot calculate region-based income")
		return
	
	if map_generator == null:
		push_error("[PlayerManager] CRITICAL: MapGenerator is null - cannot access region data")
		return
	
	# Get all regions owned by this player
	var owned_regions = region_manager.get_player_regions(player_id)
	print("[PlayerManager] Player ", player_id, " owns ", owned_regions.size(), " regions: ", owned_regions)
	
	if owned_regions.is_empty():
		print("[PlayerManager] No regions owned by ", player.get_player_name(), " - no resource income")
		return
	...
	if regions_node == null:
		push_error("[PlayerManager] CRITICAL: No Regions node found in map generator")
		return
	...
	for region_id in owned_regions:
		...
		if region_node != null:
			# Add region resource production
			...
			# Debug output for population income
			if population_gold > 0:
				print("[PlayerManager] Region ", region_node.get_region_name(), " (pop: ", region_node.get_population(), 
					", level: ", region_node.get_region_level(), ") generates ", population_gold, " gold from population")
	...
	print("[PlayerManager] Applied region-based income for ", player.get_player_name(), 
		" from ", owned_regions.size(), " regions (total value: ", total_income_value, ")")
...
func give_test_resources(player_id: int) -> void:
	...
	print("[PlayerManager] Gave test resources to ", player.get_player_name())
...
func temp_deduct_resources(cost: Dictionary) -> void:
	"""Temporarily deduct resources (for UI preview) - not implemented yet"""
	# This would need to store the deducted amounts and restore them
				...
				for region_id in owned_regions:
					...
					if region_node != null:
						...
						if garrison != null:
							...
							if garrison_food_cost > 0:
								print("[PlayerManager] Region ", region_node.get_region_name(), " garrison food cost: ", garrison_food_cost)
		...
		for army in all_armies:
			if army.get_player_id() == player_id:
				...
				if army_composition != null:
					...
					if army_food_cost > 0:
						print("[PlayerManager] Army ", army.name, " food cost: ", army_food_cost)
