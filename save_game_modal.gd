extends Control
class_name SaveGameModal

signal back_requested(context: int)
...
var current_context: Context = Context.GAME_MENU
...
func configure(mode: Mode, entries: Array[Dictionary], context: Context = Context.GAME_MENU) -> void:
...
func get_selected_file_name() -> String:
	return selected_file_name

func get_entered_file_name() -> String:
	return file_name_input.text.strip_edges()
...
func _on_back_pressed() -> void:
	if current_context == Context.MAIN_MENU:
		visible = false
		var main_menu: MainMenu = get_tree().current_scene as MainMenu
		if main_menu.USE_DEMO_MENU:
			main_menu._show_demo_menu()
		else:
			main_menu._show_main_menu()
		get_viewport().set_input_as_handled()
		return
	get_viewport().set_input_as_handled()
	back_requested.emit(int(current_context))

func _on_action_pressed() -> void:
	action_requested.emit(int(current_mode), selected_file_name, get_entered_file_name())
