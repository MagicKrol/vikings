...
# Budget management system for recruitment
func collect_recruitment_requests(player_id: int) -> Array[Army]:
	"""Collect all armies for a player that have requested recruitment"""
	var requesting_armies: Array[Army] = []
	if armies_by_player.has(player_id):
		for army in armies_by_player[player_id]:
			if is_instance_valid(army) and army.is_recruitment_requested():
				requesting_armies.append(army)
	return requesting_armies

func assign_recruitment_budgets(player_id: int, player: Player) -> void:
	"""Assign equal budget splits to all armies that requested recruitment"""
	var requesting_armies = collect_recruitment_requests(player_id)
	if requesting_armies.is_empty():
		print("[ArmyManager] No armies requesting recruitment for player ", player_id)
		return
	
	# Use BudgetManager to allocate player resources directly
	# Note: BudgetManager now assigns directly to army.assigned_budget, no return value needed
	var budget_manager = BudgetManager.new()
	var assigned_count = budget_manager.allocate_recruitment_budgets(requesting_armies, player, region_manager)
	print("[ArmyManager] BudgetManager assigned budgets to ", assigned_count, " armies at castles")

func execute_recruitment_for_player(player_id: int, need_key: String = "game_start") -> void:
	"""Execute recruitment for all armies with assigned budgets"""
	if not armies_by_player.has(player_id):
		return
	...
	for army in armies_by_player[player_id]:
		...
		var budget = army.get_assigned_budget()
		if budget != null:
			print("[ArmyManager] Executing recruitment for army ", army.name)
			var result = recruitment_manager.hire_soldiers(army, need_key, budget)
			print("[ArmyManager] Recruitment result for ", army.name, ": hired=", result.get("hired", {}))
			
			# Clear the recruitment request after processing
			army.clear_recruitment_request()

func flag_armies_needing_recruitment(player_id: int, turn_number: int = 1) -> int:
	"""Flag armies that need recruitment based on power threshold. Returns number of armies flagged."""
	var flagged_count = 0
	if armies_by_player.has(player_id):
		for army in armies_by_player[player_id]:
			if is_instance_valid(army) and army.needs_recruitment(turn_number):
				army.request_recruitment()
				flagged_count += 1
				print("[ArmyManager] Flagged army ", army.name, " for recruitment (power: ", army.get_army_power(), ")")
	return flagged_count

func process_recruitment_cycle(player_id: int, player: Player, turn_number: int = 1) -> void:
	"""Complete recruitment cycle: flag needy armies, split budget, execute recruitment"""
	# Step 1: Flag armies that need recruitment
	var flagged_count = flag_armies_needing_recruitment(player_id, turn_number)
	print("[ArmyManager] Flagged ", flagged_count, " armies for recruitment")
	
	# Step 2: Assign budgets to requesting armies using player resources
	assign_recruitment_budgets(player_id, player)
	
	# Step 3: Execute recruitment for armies with budgets
	execute_recruitment_for_player(player_id)
