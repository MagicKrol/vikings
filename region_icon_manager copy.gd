...
static func _place_mountain_icon(parent_pg: Polygon2D, region_data: Dictionary, polygon_scale: float, map_size_scale: float) -> void:
	...
	var use_random := _should_use_random_mountain(region_data)
	var icon_info := _pick_random_mountain_icon() if use_random else _pick_mountain_variant(analysis)
	...
	var scale_factor := _compute_mountain_scale(analysis, icon_info, use_random, polygon_scale)
	icon.scale = Vector2(scale_factor, scale_factor)
	icon.z_index = parent_pg.z_index + 10
	icon.set_meta("mountain_icon", true)
	parent_pg.add_child(icon)

static func _should_use_random_mountain(region_data: Dictionary) -> bool:
	if bool(region_data.get("has_non_mountain_neighbor", false)):
		return false
	return not bool(region_data.get("coast", false))
...
static func _compute_mountain_scale(analysis: Dictionary, icon_info: Dictionary, randomize_scale: bool, polygon_scale: float) -> float:
	if randomize_scale:
		var rng := _get_rng()
		var jitter: float = rng.randf_range(0.7, 0.9)
		return MOUNTAIN_ICON_BASE_SCALE * polygon_scale * jitter
