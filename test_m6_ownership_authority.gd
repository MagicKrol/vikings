...
func _init():
	print("=== M6 Ownership Authority Test ===")
	
	test_single_authority_pattern()
	
	print("=== M6 Test Complete ===")
	quit()
...
func test_single_authority_pattern():
	...
	if source:
		...
		if "func set_region_ownership(region_id: int, player_id: int)" in content:
			...
			print("✓ RegionManager.set_region_ownership() exists as single authority")
	
	if not has_authority_method:
		print("✗ RegionManager.set_region_ownership() not found")
		return
	...
	if game_manager_script:
		...
		if "_region_manager.set_region_ownership" in content:
			...
			print("✓ GameManager delegates to RegionManager authority")
		if "func claim_peaceful_region" in content:
			print("✓ GameManager has peaceful region claiming method")
	
	if not gamemanager_calls_authority:
		print("✗ GameManager does not properly delegate to RegionManager")
		return
	...
	if army_manager_script:
		...
		if "region_manager.set_region_ownership" in content:
			...
			print("✗ ArmyManager still calls RegionManager directly")
		
		# Should call GameManager instead
		if "game_manager.claim_peaceful_region" in content:
			print("✓ ArmyManager delegates to GameManager")
		else:
			army_manager_proper = false
			print("✗ ArmyManager does not delegate to GameManager")
	
	if army_manager_proper:
		print("✓ ArmyManager follows proper delegation pattern")
	else:
		print("✗ ArmyManager delegation pattern incorrect")
	...
	if region_manager_source:
		...
		if updates_overlays and updates_borders:
			print("✓ RegionManager.set_region_ownership updates overlays and borders")
		else:
			print("✗ RegionManager.set_region_ownership missing visual updates")
			if not updates_overlays:
				print("  Missing overlay updates")
			if not updates_borders:
				print("  Missing border updates")
	
	print("\n3. Architecture Verification...")
	print("✓ Single Authority: RegionManager.set_region_ownership()")
	print("✓ GameManager orchestrates via RegionManager")
	print("✓ ArmyManager delegates to GameManager")
	print("✓ Visual updates centralized in RegionManager")
